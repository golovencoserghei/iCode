...
### Исправлено (критический разбор — корректность анализа, Sprint 1)
- **dead-code больше не флагает роут-хендлеры.** `find_dead_code` кросс-ссылается на
  таблицу `routes` и исключает методы-хендлеры маршрутов (достижимы через URL, не по
  имени). На реальном Laravel: 10+ ложных срабатываний → 0.
- **`get_callers`/`get_callees` — ООП-резолв вместо чистого имени.** Каждое ребро
  получает `resolution` (own/inherited/exact/by_name) + `target_class` (по
  receiver+иерархии); параметр `class` разруливает коллизию одноимённых методов
  (`get_callers("store", class="UserController")` — в проекте 13 классов с `store`);
  при коллизии без `class` — предупреждение в `_meta.note`.
- **routing-полнота:** `Route::resource`/`apiResource` разворачиваются в экшены
  (index/store/show/update/destroy[/create/edit], с учётом `->only()`/`->except()`),
  `Route::match([...])` → маршрут на метод, `Route::view`/`redirect`. На реальном
  Laravel: 141 → 148 маршрутов. Строковые хендлеры нормализуются до короткого имени.
- **PHP type-inference (Sprint 2):** call-граф резолвится не только по `$this`, но и
  по типу переменной — типизированные параметры (`User $u`, promoted-параметры
  конструктора) и `$x = new X()` → `$u->save()` резолвится к `User::save`. Замыкания
  имеют собственный скоуп типов (без протечек/шедоуинга). На реальном Laravel:
  разрешённых member-вызовов 1432 → 2010 (+578; неразрешённых `$var` −33%).
- **PHP typed-свойства (Sprint 3):** `$this->repo->find()` резолвится к `UserRepo::find`
  по типу свойства класса — типизированные `property` и promoted-параметры конструктора.
  Доступны и в замыканиях метода (`$this` привязан; static-замыкания исключены).
  На реальном Laravel: разрешённых вызовов 2010 → 2424 (+414).
- **Глубина на Python/JS/TS (#4):** call-граф этих языков теперь захватывает
  получателя вызова (`self`/`cls`/`this`/класс/переменная). Унифицирован разделитель
  qualified_name (`split_qualified`: `Class::method` и `Class.method`) — OOP-модель,
  резолв, `find_dead_code`/`find_unreachable` работают и для Python/JS/TS. Python
  `self.save()` резолвится к классу. На реальном Python (onyx): доля вызовов с
  получателем 0% → 36%.
- **`find_unreachable` (#2):** недостижимый код через обход call-графа (`WITH RECURSIVE`)
  от точек входа (маршруты, main/handle/boot/тесты). Ловит «мёртвые кластеры» —
  функции, вызываемые только из другого мёртвого кода (их имя есть среди callee,
  поэтому `find_dead_code` их пропускает). Консервативно по имени; кандидаты для ревью
  (не видит рефлексию/динамику). CLI: `icode unreachable [--language]`.
- **`quality_harness.py` (#6):** воспроизводимый scorecard КОРРЕКТНОСТИ ответов против
  ground truth (не токены-прокси) — routing/inheritance/резолв/find_existing/unreachable.
