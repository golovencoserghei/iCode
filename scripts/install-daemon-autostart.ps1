# Установка автозапуска фонового демона icode через Scheduled Task (Windows).
#
# Скрипт создаёт задачу, которая при входе текущего пользователя запускает
# `icode.exe daemon run` без консольного окна. Параметры триггера — только
...
#
# Параметры:
#   -BinaryPath   — путь к icode.exe (по умолчанию: release-сборка рядом)
#   -TaskName     — имя задачи (по умолчанию iCodeDaemon)
...
function Resolve-BinaryPath {
    ...
    $candidate = Join-Path $scriptDir "..\target\release\icode.exe"
...
# Если пользователь указал -ICodeHome — установить пользовательскую
...
# тоже используют её.
if ($ICodeHome -ne "") {
    if (-not (Test-Path $ICodeHome)) {
        New-Item -ItemType Directory -Path $ICodeHome -Force | Out-Null
        Write-Host "Создана папка: $ICodeHome"
    }
    # setx пишет в HKCU\Environment, новое значение видно в новых консолях.
    [System.Environment]::SetEnvironmentVariable("ICODE_HOME", $ICodeHome, "User")
    Write-Host "ICODE_HOME установлен (user-level): $ICodeHome"

    # Проверим, есть ли daemon.toml — если нет, создадим минимальный шаблон.
    $cfg = Join-Path $ICodeHome "daemon.toml"
...
# Действие: запустить icode.exe daemon run. Без окна консоли.
...
Register-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "Фоновый демон iCode (индексация кодовых папок в режиме one-writer-many-readers)." | Out-Null

Write-Host "Задача '$TaskName' создана."
Write-Host "Триггер: при входе пользователя $env:USERNAME."

if ($StartNow) {
    # Запускаем демон напрямую (не через Scheduled Task), чтобы ICODE_HOME
    ...
    $env:ICODE_HOME = if ($ICodeHome -ne "") { $ICodeHome } else { $env:ICODE_HOME }
    Start-Process -FilePath $exe -ArgumentList "daemon run" -WindowStyle Hidden
    Write-Host "Демон запущен прямо сейчас (PID см. в 'icode daemon status')."
} else {
