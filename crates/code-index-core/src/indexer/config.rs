...
impl IndexConfig {
    /// Загрузить конфигурацию из .icode/config.json.
    /// Если файл не существует — вернуть конфиг по умолчанию.
    pub fn load(project_root: &Path) -> Result<Self> {
        let config_path = project_root.join(".icode").join("config.json");
    ...
    pub fn save(&self, project_root: &Path) -> Result<()> {
        let config_dir = project_root.join(".icode");
    ...
    pub fn build_file_exclude_matcher(&self) -> GlobSet {
        ...
        for pat in &self.exclude_file_patterns {
            match Glob::new(pat) {
                Ok(g) => { builder.add(g); }
                Err(e) => {
                    warn!("[config] некорректный exclude_file_pattern '{}': {}", pat, e);
                }
            }
        }
        builder.build().unwrap_or_else(|e| {
            info!("[config] GlobSetBuilder.build failed: {}", e);
            GlobSet::empty()
        })
    }
}
