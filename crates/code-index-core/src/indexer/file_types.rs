...
const TEXT_EXTENSIONS: &[&str] = &[
    ...
    "json", "yaml", "yml", "toml",
    "xml", "css",
    "c", "h", "cpp", "hpp", "cs", "rb", "swift", "kt",
    "csv", "env", "ini", "cfg",
    "sql", "sh", "bash", "zsh", "fish", "bat", "ps1",
];

/// Имена файлов без расширения, которые индексируются как текст.
const NAMED_TEXT_FILES: &[&str] = &[
    "makefile", "dockerfile", "gemfile", "rakefile", "procfile",
    "vagrantfile", "brewfile", "jenkinsfile",
    ".gitignore", ".gitattributes", ".editorconfig",
    ".env", ".env.example", ".env.local", ".env.test", ".env.production",
    "readme", "license", "contributing", "changelog", "authors",
];

/// Маппинг расширений text-файлов на конкретное имя языка.
/// Используется при записи в БД: вместо безликого "text" храним "sql", "shell" и т.д.
pub fn text_language_for_ext(ext: &str) -> &'static str {
    match ext {
        "sql"                          => "sql",
...
pub fn categorize_file(path: &Path) -> FileCategory {
    ...
    // Файлы без расширения: проверяем по имени файла
    if ext.is_empty() {
        let name = path
            .file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("")
            .to_lowercase();
        if NAMED_TEXT_FILES.contains(&name.as_str()) {
            return FileCategory::Text;
        }
    }
