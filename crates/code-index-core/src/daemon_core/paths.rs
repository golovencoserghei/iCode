...
//
// Единый источник истины — переменная окружения `ICODE_HOME`.
...
//   * Windows:  setx ICODE_HOME "C:\tools\code-index"
//   * Linux:    export ICODE_HOME="$HOME/.local/code-index"
//   * macOS:    launchctl setenv ICODE_HOME /Users/you/code-index
//   * MCP:      "env": { "ICODE_HOME": "..." } в .mcp.json
...
pub const HOME_ENV: &str = "ICODE_HOME";

/// Прочитать `ICODE_HOME`. Возвращает ошибку с понятным сообщением,
...
/// Путь к файлу конфигурации демона: `$ICODE_HOME/daemon.toml`.
pub fn config_path() -> Result<PathBuf> {
    Ok(home_dir()?.join("daemon.toml"))
}

/// Путь к каталогу runtime-состояния: та же папка, что и `ICODE_HOME`.
...
/// Убедиться, что каталог `ICODE_HOME` существует.
