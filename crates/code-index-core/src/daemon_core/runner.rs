...
use crate::graph_store::{GraphStore, GraphStoreHandle};
...
pub async fn run(processor_registry: Option<Arc<ProcessorRegistry>>) -> Result<()> {
    ...
    info!("[daemon] HTTP health-IPC: http://{}", actual_addr);
    ...
    let server_handle = tokio::spawn(async move {
        if let Err(e) = axum::serve(listener, router).await {
            warn!("[daemon] HTTP-сервер упал: {}", e);
        }
    });
    ...
    // Граф-слой (v0.10+): если в daemon.toml есть секция [graph] — подключаемся
    // и запускаем фоновый воркер. Иначе все workers получают disabled-handle.
    let graph_handle = if let Some(graph_cfg) = cfg.graph.as_ref() {
        if graph_cfg.password.is_empty() {
            warn!("[daemon] граф-БД: password не задан");
        }
        match GraphStore::connect(&graph_cfg.bolt_url, &graph_cfg.username, &graph_cfg.password).await {
            Ok((store, handle)) => {
                tokio::spawn(store.run());
                info!("[daemon] граф-БД: подключён к {}", redact_url(&graph_cfg.bolt_url));
                handle
            }
            Err(e) => {
                warn!("[daemon] граф-БД: ошибка подключения к {} — {}", redact_url(&graph_cfg.bolt_url), e);
                info!("[daemon] граф-БД: dual-write выключен, продолжаем без него");
                GraphStoreHandle::disabled()
            }
        }
    } else {
        GraphStoreHandle::disabled()
    ...
    loop {
        tokio::select! {
            ...
            _ = tokio::signal::ctrl_c() => {
                info!("[daemon] Ctrl-C — завершение");
                break;
            }
        }
    }

    info!("[daemon] остановка worker'ов...");
...
async fn handle_reload(
    ...
    // Запускаем worker'ы для добавленных. Семафор разделяется с основным run(),
    // чтобы новые workers не превышали общий лимит initial-reindex'ов.
    let indexer_section = cfg.indexer.clone();
...
/// либо `ICODE_HOME` не задана (значит и запускать негде).
